...
class TemplatePromptGenerator4IC1(TemplatePromptGenerator):
        ...
        if self.code_summary_type == 1:
            ...
            if(increment_address):
                return (
                    ...
                    "where the integer represents a RISC-V instruction, next loaded into instruction memory.\n"
                )
            else:
                return (
                    "You will receive a description of bins (i.e. test cases) of a testbench for "
    ...
    def _load_examples(self) -> str:
        if(self.few_shot == 1):
            examples = (
                ...
                f"- 0x00718a33 => funct7=0000000 rs2=00111 rs1=00011 funct3=000 rd=10100 op=0110011 => add_seen covered\n"
                f"- 0xffdff06f => imm=-4 rd=00000 op=1101111 => jal_seen, jal_br_backwards, jal_zero_dst covered\n"
                f"- 0x000292b3 => funct7=0000000 rs2=00000 rs1=00101 funct3=001 rd=00101 op=0110011 => sll_seen, sll_zero_src covered\n"
                f"- 0x00f7a2a3 => imm=5 rs2=01111 rs1=01111 funct3=010 op=0100011 => sw_seen, sw_same_src covered\n"
                f"- 0x00101023 => imm=0 rs2=00001 rs1=00000 funct3=001 op=0100011 => sh_seen, sh_zero_src covered\n"
                f"- 0x020b1b3, 0x0051c233 => funct7=0000000 rs2=00010 rs1=00001 funct3=011 rd=00011 op=0110011, funct7=0000000 rs2=00101 rs1=00011 funct3=100 rd=00100 op=0110011 => sltu_seen, xor_seen, sltu->xor_raw_hazard covered\n"
                f"- 0x006080ef, 0x00112023 => imm=32774 rd=00001 op=1101111, imm=0 rs2=00001 rs1=00010 funct3=010 op=0100011 => jal_seen, jal_br_forwards, sw_seen, jal->sw_raw_hazard covered\n"
                f"------\n"  
            )
        else:
            examples = ""
    ...
    def _load_init_question(self) -> str:
        if(increment_address):
            init_question = (
                ...
                f"execution from the current PC. Make sure the instructions are valid RISC-V instruction codes."
                f"Remember that one instruction is 32 bits (8 bytes).\n"
            )
        else:
            init_question = (
    ...
    def _load_result_summary(self, **kwargs) -> str:
        if kwargs["response_invalid"]:
            ...
            if(increment_address):
                result_summary = (
                    "Your list of updates was invalid, either because the instructions you provided are not valid R-type, S-type, or J-type RISC-V "
...
class TemplatePromptGenerator4IC2(TemplatePromptGenerator4IC1):
    ...
    def generate_initial_prompt(self, **kwargs) -> str:
        ...
        if(increment_address):
            ...
            prompt += self._load_examples()
            prompt += (
                f"Following the bins description, generate a list, which can be empty if "
                f"necessary, of ainstructions in 32-bit hexadecimal format "
                f"to update the CPU's memory, ensuring it covers the specified bins upon resuming "
