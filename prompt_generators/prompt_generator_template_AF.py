...
class TemplatePromptGeneratorAF(TemplatePromptGenerator):
    def __init__(
        self,
        dut_code_path: str = "",
        tb_code_path: str = "",
    ...
    def _load_coverage_difference_prompts_dict(self) -> Dict[str, str]:
        coverage_difference_template = {
            f"full_read_wrap": "the read pointer wraps to 0.\n",
            f"gray_read_wrap": "the MSB of the read pointer toggles.\n",
            f"underflow": "a read operation is requested while the FIFO is empty.\n",
            f"empty": "the FIFO is empty.\n",
            f"full_write_wrap": "the write pointer wraps to 0.\n",
            f"gray_write_wrap": "the MSB of the write pointer toggles.\n",
            f"overflow": "a write operation is requested while the FIFO is full.\n",
            f"full": "the FIFO is full.\n",
            f"read_while_write": "on read clock edge a read operation is performed, while a write operation is being requested as well.\n",
            f"write_while_read": "on write clock edge a write operation is performed, while a read operation is being requested as well.\n"
        }
